...
<?php
$tripids = array(); // Array to store unique tripids

foreach ($logs as $log) {
  $tripid = $log->tripid;
  if (!in_array($tripid, $tripids)) {
    $tripids[] = $tripid; // Add unique tripid to the array
  }
}
?>
    ...
    <?php foreach ($tripids as $tripid): ?>
      <div class="table-responsive mb-4">
        ...
        <div class="text-center">
          <a href="map.php?tripid=<?= $tripid ?>" class="btn btn-light border-dark">Tampilkan Peta</a>
        </div>
      </div>
      <hr>
    <?php endforeach; ?>
