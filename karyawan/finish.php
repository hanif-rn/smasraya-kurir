<?php
    ...
    $sql = "UPDATE users SET trips = trips + 1 WHERE username = :username";
    $statement = $connection->prepare($sql);
    $statement->bindParam(':username', $username, PDO::PARAM_STR);
    $statement->execute();
    
    
    echo "Anda telah menyelesaikan perjalanan pada tanggal-waktu " . $time . " di koordinat " . $latitude . ", " . $longitude;
    exit;
?>
